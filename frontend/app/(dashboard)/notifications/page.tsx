  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <button onClick={handleMarkAllRead}
            className="flex items-center gap-2 px-4 py-2 border border-gray-200 text-sm font-medium text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">
      ...
      <div className="space-y-2">
        {isLoading ? (
          [1, 2, 3, 4].map(n => <div key={n} className="h-16 bg-gray-100 rounded-2xl animate-pulse" />)
          ...
          <div
            ...
            className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-colors ${
              n.isRead ? 'bg-white border-gray-100' : 'bg-pink-50 border-pink-100'
            }`}
          >
            <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${PRIORITY_DOT[n.priority] ?? 'bg-gray-300'}`} />
